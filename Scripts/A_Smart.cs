...
///Krzysztof :: Update1 ::
/// 1. Blue tank renamed according to the documentation.
/// 2. Blue Tank script raplaced to A_Smart (Previously DumbTank)
/// 3. Camping - (Hide and wait for enemy) Smarter AI
/// </summary>
public class A_Smart : AITank
{
    ...
    private void AttackEnemyTank()
    {
        ...
        if (enemyTank != null)
        {
            if (Vector3.Distance(transform.position, enemyTank.transform.position) < 100f)
            {
                TurretFireAtPoint(enemyTank);
            }
            else
            {
                FollowPathToWorldPoint(enemyTank, 0.2f, heuristicMode);
            }
        }
    }
    ...
    private void Camping()
    {
        ...
        if (enemyTank != null)
        {
            if (Vector3.Distance(transform.position, enemyTank.transform.position) < 125f)
            {
                TurretFireAtPoint(enemyTank);
            }
            else
            {
                FollowPathToWorldPoint(enemyTank, 100f, heuristicMode);
            }
        }
    }
